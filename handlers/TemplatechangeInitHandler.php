...
class TemplatechangeInitHandler {
	...
	public function smartyInit(Smarty $hook_data) {
		...
		$plugin_templates = PLUGINS_DIR . DIRECTORY_SEPARATOR . LMSTemplatechangePlugin::plugin_directory_name . DIRECTORY_SEPARATOR . 'templates';
	...
	public function ModulesDirInit(array $hook_data = array()) {
		$plugin_modules = PLUGINS_DIR . DIRECTORY_SEPARATOR . LMSTemplatechangePlugin::plugin_directory_name . DIRECTORY_SEPARATOR . 'modules';
	...
	public function menuInit(array $hook_data = array()) {
		$hook_data['finances']['submenu'][] = array(
			'name' => trans('Invoice template'),
			'link' => '?m=templatechange',
			'tip' => trans('Change invoice template'),
			'prio' => 130,
		);
	...
	public function accessTableInit() {
		...
		$access->insertPermission(new Permission('template_change', trans('invoices template changing'), '^templatechange$'),
